...
其中 $$D_{i,j}=\|G_{\boldsymbol{W}}(X_{i}) - G_{\boldsymbol{W}}(X_{j})\|_{2}$$ 为映射向量之间的欧式距离，hinge 函数 $[z]_+=\max(z,0)$，$m$ 为阈值参数。该公式表明我们希望**相似样本之间的距离尽可能小，而不相似样本之间的距离要大于阈值m**。
